...
- **CSRF:** Sessions are cookies (`httpOnly`, `path: "/"`, `SameSite=Lax`, `Secure` in prod — keep these explicit in `makeAuth` via `advanced.defaultCookieAttributes` / `useSecureCookies`). Real protection for mutating `/api/*` routes (POST/PUT/PATCH/DELETE) comes from requiring an `Origin`/`Referer` matching `BETTER_AUTH_URL`'s origin (`originOk` in `server/index.ts`). **Do not set `SameSite=None` or remove the Origin check without adding a real CSRF defense.** No CORS is configured for API reads, but the browser may still send cross-origin credentialed mutations; the `Origin`/`Referer` validation is what enforces CSRF defense.
...
- **Export DoS:** `/api/export` caps aggregate export input bytes at `MAX_EXPORT_BYTES` (32 MB) and returns 413 early; `zipSync` materializes the archive in memory, so an unbounded set can OOM a 128 MB isolate.
