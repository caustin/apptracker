...
export function Funnel({ positions }: { positions: Position[] }) {
  ...
  if (counts[0] === 0) {
    return (
      <div className="empty-state">
        Nothing in the funnel yet — mark a position as applied and it will show up here.
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${LABEL_W + CHART_W} ${height}`} className="funnel" role="img"
        aria-label="Job search funnel by stage">
        {STAGES.map((stage, i) => {
          const y = i * (BAND_H + GAP);
          const w = width(counts[i]);
          const dark = i >= 2;
          const pct = i > 0 && counts[i - 1] > 0 ? Math.round((counts[i] / counts[i - 1]) * 100) : null;
          return (
            <g key={stage.label}>
              {i > 0 && (
                <>
                  <path
                    ...
                    opacity="0.25"
                  />
                  <text x={cx + CHART_W / 2 - 4} y={y - GAP / 2 + 4} textAnchor="end" className="funnel-pct">
                    {pct}% →
                  </text>
                </>
              )}
              <rect x={cx - w / 2} y={y} width={w} height={BAND_H} rx="6" fill={COLORS[i]} />
              <text x={cx} y={y + BAND_H / 2 + 6} textAnchor="middle"
                className={`funnel-count ${dark ? 'on-dark' : ''}`}>
                {counts[i]}
              </text>
              <text x={LABEL_W - 16} y={y + BAND_H / 2 + 5} textAnchor="end" className="funnel-label">
                {stage.label}
              </text>
            </g>
          );
      ...
      <p className="funnel-footnote">
        {leads > 0 && <span>{leads} lead{leads === 1 ? '' : 's'} not yet applied · </span>}
        {rejected + withdrawn > 0
          ? <span>{rejected + withdrawn} closed ({rejected} rejected, {withdrawn} withdrawn)</span>
